...
public static class qb_Utility
{
	...
	public static string GetHeadDirectory()
	{
		...
		//have to create an object to get its asset path - there has to be a better way
		qb_UtilityObject utilityObject = ScriptableObject.CreateInstance<qb_UtilityObject>();
		string thisDir = AssetDatabase.GetAssetPath(MonoScript.FromScriptableObject(utilityObject));		
		
		if(thisDir != string.Empty)
		{
			//thisDir = thisDir.Replace("/Editor/qb_UtilityObject.cs","");
			int lastSlash = thisDir.LastIndexOf("QuickBrush/");
			
			if(lastSlash > 0)
				thisDir = thisDir.Substring(0,lastSlash + 10);
		}
		
		ScriptableObject.DestroyImmediate(utilityObject);
