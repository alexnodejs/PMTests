...
#region Initialization
		...
		void OnGUI()
		{
			...
			EditorGUILayout.BeginHorizontal();
				...
				if(!modMode)
				{
					newGroupName = EditorGUI.TextField(new Rect(3, 27, Screen.width-50, 17), newGroupName);
			...
			if(groupContainer.sceneGroups != null)
			{
				...
				for(int i = 0; i < groupContainer.sceneGroups.Length; i++)
				{
					...
					{
						Rect rowRect = new Rect(r.x, r.y + r.height + 2, Screen.width-6, 20);
					...
					if( GUILayoutUtility.GetLastRect().Contains(curEvent.mousePosition) )
					{
						if(dragging)
						{
							dragIndex = i;
